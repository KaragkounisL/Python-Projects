...
You have a car with an unlimited gas tank and it costs B[i] of gas to travel from station i to its next station (i+1). You begin the journey with an empty 
...
def canCompleteCircuit(self, A, B):
    ...
    for i in range(len(A)):
        ...
        if tank < 0:
            start = i + 1
            tank = 0
    return start if sum(A) >= sum(B) else -1
